...
export class ReactOpenApiGen {
  ...
  generate(): void {
    ...
    try {
      ...
      const modelImports =
        this.globals.modelIndexFile || this.options.indexFile ? models.map((m) => new Import(m.name, './models', m.options)) : null;
 ...
 */
export async function runReactOpenApiGen() {
  ...
  try {
    ...
    const gen = new ReactOpenApiGen(openApi, options);
    gen.generate();
  } catch (err) {
