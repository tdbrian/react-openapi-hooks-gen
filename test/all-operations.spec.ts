...
const gen = new ReactOpenApiGen(allOperationsSpec as OpenAPIObject, options);
gen.generate();
...
describe("Generation tests using all-operations.json", () => {
  ...
  it("GET /path1", () => {
    ...
    if (success) {
      ...
      const json = success.content.find(
    ...
    const respDefault = operation.allResponses.find(
  ...
  it("POST /path1", () => {
    ...
    if (operation.requestBody) {
      ...
      const plain = operation.requestBody.content.find(
        (c) => c.mediaType === "text/plain"
      );
  ...
  it("PUT /path4", () => {
    ...
    if (operation.requestBody) {
      ...
      const _any = operation.requestBody.content.find(
        (c) => c.mediaType === "*/*"
      );
  ...
  it("GET /path5", () => {
    ...
    if (success) {
      ...
      expect(json).toBeDefined();
      const resp200 = operation.allResponses.find(
        (r) => r.statusCode === "200"
      );
      expect(resp200).toBe(success);
    }
