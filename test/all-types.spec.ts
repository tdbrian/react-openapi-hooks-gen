...
const gen = new ReactOpenApiGen(
  allTypesSpec as OpenAPIObject,
  options as Options
);
gen.generate();
...
it("Api", (done) => {
  ...
  if (api) {
    ...
    parser.parseSource(ts).then((ast) => {
      ...
      expect(
        ast.imports.find(
          (i) => i.libraryName === "../models/referenced-in-service-one-of-1"
        )
      )
      ...
      expect(
        ast.imports.find(
          (i) => i.libraryName === "../models/referenced-in-service-one-of-2"
        )
      )
        .withContext("ref2 import")
        .toBeDefined();
      expect(
        ast.imports.find((i) => i.libraryName === "../models/a/b/ref-object")
      )
      ...
      expect(
        ast.imports.find(
          (i) => i.libraryName === "../models/referenced-in-one-of"
        )
      )
...
describe("Generation tests using all-types.json", () => {
  ...
  it("disjunct model", (done) => {
    ...
    parser.parseSource(ts).then((ast) => {
      ...
      expect(
        ast.imports.find(
          (i) => i.libraryName === "./referenced-in-nullable-one-of"
        )
      )
