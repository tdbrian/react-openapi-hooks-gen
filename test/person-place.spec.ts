...
import { ReactOpenApiGen } from '../lib/react-openapi-gen';
import options from './person-place.config.json';
import personAndPlaceSpec from './person-place.json';

const gen = new ReactOpenApiGen(personAndPlaceSpec, options);
gen.generate();
...
describe('Generation tests using person-place.json', () => {
  ...
  it('Entity model', (done) => {
    ...
    const parser = new TypescriptParser();
    parser.parseSource(ts).then((ast) => {
      expect(ast.imports.find((i) => i.libraryName === './pp-id-model'))
  ...
  it('Person model', (done) => {
    ...
    const parser = new TypescriptParser();
    parser.parseSource(ts).then((ast) => {
      expect(ast.imports.find((i) => i.libraryName === './pp-entity-model'))
        .withContext('entity import')
        .toBeDefined();
      expect(ast.imports.find((i) => i.libraryName === './pp-person-place-model'))
        .withContext('person-place import')
        .toBeDefined();
      expect(ast.declarations.length).toBe(1);
      expect(ast.declarations[0]).toEqual(jasmine.any(InterfaceDeclaration));
      const decl = ast.declarations[0] as InterfaceDeclaration;
  ...
  it('PersonPlace model', (done) => {
    ...
    const parser = new TypescriptParser();
    parser.parseSource(ts).then((ast) => {
      expect(ast.imports.find((i) => i.libraryName === './pp-place-model'))
        .withContext('place import')
        .toBeDefined();
      expect(ast.declarations.length).toBe(1);
      expect(ast.declarations[0]).toEqual(jasmine.any(InterfaceDeclaration));
      const decl = ast.declarations[0] as InterfaceDeclaration;
